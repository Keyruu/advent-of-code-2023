    ...
    two_d.iter().enumerate().for_each(|(i, line)| {
        line.iter().enumerate().for_each(|(j, char) | {
            if special_chars.contains(*char) {
                ...
                sum_of_parts += get_sum_of_parts_in_line(above_line, j);
                sum_of_parts += get_sum_of_parts_in_line(line, j);
                sum_of_parts += get_sum_of_parts_in_line(below_line, j);
...
fn get_sum_of_parts_in_line(line: &Vec<char>, index: usize) -> u32 {
    let mut sum = 0;
    ...
    if all_number {
        sum += left.to_digit(10).unwrap() * 100
    ...
    if left_and_right_number || only_left_number {
        if line.get(index - 2).unwrap().is_numeric() == false {
            ...
            sum += line.get(index - 2).unwrap().to_digit(10).unwrap() * 10
                + left.to_digit(10).unwrap()
        } else {
            sum += line.get(index - 3).unwrap_or(&'0').to_digit(10).unwrap_or(0) * 100
    ...
    if left_and_middle_number {
        sum += line.get(index - 2).unwrap_or(&'0').to_digit(10).unwrap_or(0) * 100
    ...
    if left_and_right_number || only_right_number {
        if line.get(index + 2).unwrap().is_numeric() == false  {
            sum += right.to_digit(10).unwrap()
        } else if line.get(index + 3).unwrap().is_numeric() == false {
            sum += right.to_digit(10).unwrap() * 10
                + line.get(index + 2).unwrap().to_digit(10).unwrap()
        } else {
            sum += right.to_digit(10).unwrap() * 100
